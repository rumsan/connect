...
export const getAddressValidator = (validationType: ValidationAddress) => {
  const validators = {
    ...
    [ValidationAddress.EMAIL]: (address: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(address),
    [ValidationAddress.PHONE]: (address: string) => /^\+?[1-9]\d{1,14}$/.test(address),
  };
...
export const getContentValidator = (validationType: ValidationContent) => {
  const validators = {
    [ValidationContent.URL]: (content: string) => /^https?:\/\/[^\s$.?#].[^\s]*$/.test(content),
